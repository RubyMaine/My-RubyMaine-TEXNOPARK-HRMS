         ...
         <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor"><i class="mdi mdi-city"></i> Добавить посещаемость </h3>
                </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item active"><i class="mdi mdi-city"></i> Добавить посещаемость</li>
                    </ol>
                </div>
            </div>
            <div class="container-fluid">
                ...
                <div class="row m-b-10"> 
                    <div class="col-12">
                        <button type="button" class="btn btn-danger"><i class="mdi mdi-backspace"></i><a href="<?php echo base_url(); ?>attendance/Attendance" class="text-white"><i class="" aria-hidden="true"></i> Вернутся назад </a></button>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card card-outline-info">
                            <div class="card-header">
                                <h4 class="m-b-0 text-white"><i class="mdi mdi-city" style="color:#1976d2"></i> Добавить посещаемость </h4>
                            </div>
                            <div class="card-body">
                                    <form method="post" action="Add_Attendance" id="holidayform" enctype="multipart/form-data">
                                    <div class="modal-body">
			                                    <div class="form-group">
			                                        <label><i class="mdi mdi-account-settings" style="color:#1976d2"></i> Сотрудник: </label>
                                                <select class="form-control custom-select" data-placeholder="Choose a Category" tabindex="1" name="emid" required>
                                                   ...
                                                   <?php } else { ?>
                                                   <option value="#"> Выберите здесь cотрудник: </option>
                                            ...
                                            <label><i class="mdi mdi-calendar-multiple" style="color:#1976d2"></i> Выберите дату: </label>
                                        ...
                                        <div class="form-group" >
                                           <label class="m-t-20"><i class="mdi mdi-clock-in" style="color:#1976d2"></i> Время входа: </label>
                                            <input class="form-control" name="signin" id="single-input" value="<?php if(!empty($attval->signin_time)) { echo  $attval->signin_time;} ?>" placeholder="Выбор время входа ..." required>
                                        </div>
                                        <div class="form-group">
                                        <label class="m-t-20"><i class="mdi mdi-clock-out" style="color:#1976d2"></i> Время выхода: </label>
                                        <div class="input-group clockpicker">
                                            <input type="text" name="signout" class="form-control" value="<?php if(!empty($attval->signout_time)) { echo  $attval->signout_time;} ?>" placeholder="Выбор время выхода ..." required>
                                        </div>
                                        </div> 
                                        <div class="form-group">
                                                    <label><i class="mdi mdi-map-marker-radius" style="color:#1976d2"></i> Местонахождение: </label>
                                                <select class="form-control custom-select" data-placeholder="" tabindex="1" name="place" required>
                                                    <option value="office" <?php if(isset($attval->place) && $attval->place == "office") { echo "selected"; } ?>> Територия оффиса</option>
                                                    <option value="field"  <?php if(isset($attval->place) && $attval->place == "field") { echo "selected"; } ?>> Другой места </option>
                                                </select>
                                        </div> 
                                    </div>
                                    <div class="modal-footer">
                                    <input type="hidden" name="id" value="<?php if(!empty($attval->id)){ echo  $attval->id;} ?>" class="form-control" id="recipient-name1">                                       
                                        <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="mdi mdi-close-box"></i> Отменить </button>
                                        <button type="submit" id="attendanceUpdate" class="btn btn-success"><i class="mdi mdi-content-save-all"></i>  Сохранить </button>
                                    </div>
                                    </form>
                            </div>
                        </div>
